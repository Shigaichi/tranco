...
func TestClient_GetAuthTest(t *testing.T) {
	type fields struct {
		Username string
		Password string
	}
	tests := []struct {
		...
		{
			name:           "ok",
			fields:         fields{Username: "user", Password: "pass"},
			testStatusCode: 200,
			wantErr:        false,
		},
		{
			name:           "return error if invalid password",
			fields:         fields{Username: "user", Password: "INVALID"},
			testStatusCode: 401,
			wantErr:        true,
		},
		{
			name:           "return error if username or password is empty",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server, clean := setup(t, "", tt.testStatusCode, http.MethodGet, "/api/auth/test")
			defer clean()
			cli := &Client{
				HTTPClient: http.DefaultClient,
				BaseURL:    server.BaseURL,
			...
			err := cli.GetAuthTest(context.Background())
			if (err != nil) != tt.wantErr {
...
func TestClient_CreateList(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:                args{configuration: Configuration{Providers: []string{"crux"}, StartDate: parseDate("2021-01-01"), EndDate: parseDate("2022-01-03"), CombinationMethod: "dowdall", ListPrefix: "full", FilterPLD: "on", FilterTLD: "false", IsDailyList: true}},
			want:                CreatedList{ListId: "OK", Available: true},
			testDataPath:        "testdata/create_lists1.json",
			testStatusCode:      200,
			expectedRequestPath: "/api/lists/create",
			wantErr:             false,
		},
		{
			name:                "return error if username or password is empty",
		...
		{
			name:                "return error if username or password is empty",
			fields:              fields{Username: "username", Password: "INVALID"},
			args:                args{},
			want:                CreatedList{},
			testDataPath:        "",
			testStatusCode:      401,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server, clean := setup(t, tt.testDataPath, tt.testStatusCode, http.MethodPut, "/api/lists/create")
			defer clean()
			cli := &Client{
				HTTPClient: http.DefaultClient,
				BaseURL:    server.BaseURL,
			...
			got, err := cli.CreateList(context.Background(), tt.args.configuration)
			if (err != nil) != tt.wantErr {
				t.Errorf("CreateLists() error = %v, wantErr %v", err, tt.wantErr)
				return
			} else if tt.wantErr {
				if err == nil {
					t.Error("error was expected but not occurred")
